...
class Base(object):
	...
	def _api_response(self, endpoint, path, payload):
		""" Retrieve the requested data from Tradier. This is the main function called by other classes to retrieve
		information from the Tradier API.

		:param endpoint: The desired endpoint. By default, this is passed in from the intialization of the ``Tradier``
		class. Accepts ``'sandbox'`` or ``'api'``.
		:param path: The API path to information. By default, this is passed in from the inheriting class's endpoint.
		Endpoints are in the form ``/v1/markets/quotes``.
		:param payload: A dictionary of the information required to send to Tradier for API calls. For example, the
		Market Data endpoint requires ``{symbols: 'TSLA'}``.

		In general, this class shouldn't be accessed directly by the developer. Most classes inherit from Base and
		pass information in such as ``path``, ``endpoint``, and ``payload``.

		"""

		headers = {'Accept': 'application/json', 'Authorization': 'Bearer ' + self.__token}
		...
		raise APIException(error_type=j['fault']['detail']['errorcode'], error_message=j['fault']['faultstring'])
