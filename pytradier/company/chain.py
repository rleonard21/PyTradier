...
class Chain(Base):
	...
	def __init__(self, symbol, expiration):
		...
		.. note:: All options contracts expire on a Friday. If the requested expiration date is not a Friday, no
		data will be returned!

		Like many of the other functions PyTradier offers, ``Chain`` returns the data in the form of a dictionary with
		the options symbol as the key and the requested data as the value.
		...
		self._payload = {'symbol': self._symbol, 'expiration': self._expiration}
	...
	def symbol(self, **config):
		...
		You can then get updates on particular contracts through the ``Option`` class using the output symbols from
		this method as the input for the ``Option`` class.
